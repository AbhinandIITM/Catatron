...
from .State import State, Command, BehaviorState
from .CatatronTrot import CatatronTrot
...
class CatatronGaitControl(Node):
    def __init__(self, body, legs, imu):
        ...
        self.trotGaitController = CatatronTrot(self.default_stance,
            stance_time = 0.18, swing_time = 0.24, time_step = 0.02,
            use_imu = imu)

        self.crawlGaitController = CatatronCrawl(self.default_stance,
            stance_time = 0.55, swing_time = 0.45, time_step = 0.02)
            
        self.standController = CatatronStand(self.default_stance)

        self.Rest = CatatronRest(self.default_stance)
    ...
    def imu_orientation(self,msg):
        q = msg.orientation
        rpy_angles = tf_transformations.euler_from_quaternion([q.x,q.y,q.z,q.w])
